...
pub fn get_data(stock_name: &str, stock_type: &str) -> Vec<BoxElem> {
    ...
    let resp = match stock_type {
        "YEAR" => tokio_test::block_on(provider.get_latest_quotes("AAPL", "1d")).unwrap(),
        _ => tokio_test::block_on(provider.get_quote_history(stock_name, start, end)).unwrap(),
    };
    ...
    quotes
        ...
        .map(|(index, quote)| {
            ...
            BoxElem::new(timestamp as f64, spread)
